...
const Game1 = () => {
    ...
    const statusChange = (id, status) => {
        axios.put("http://localhost:8000/api/players/" + id, {status1: status})
            ...
            <Table striped bordered>
                ...
                <tbody>
                {
                    players.map((info, index) => {
                        return(<tr key={index}><td>{info.name}</td><td><Button style={{marginLeft: '30px', marginRight: '30px', width: '150px'}} variant={info.status1 === 'playing' ? 'success' : 'secondary'} onClick={(e)=>statusChange(info._id, 'playing')}>Playing</Button><Button  style={{marginLeft: '10px', marginRight: '30px', width: '150px'}} variant={info.status1 === 'not playing' ? 'danger' : 'secondary'} onClick={(e)=>statusChange(info._id, 'not playing')}> Not Playing</Button>
                            <Button  style={{marginLeft: '10px', marginRight: '30px', width: '150px'}} variant={info.status1 === 'undecided' ? 'warning' : 'secondary'}  onClick={(e)=>statusChange(info._id, 'undecided')}>Undecided</Button></td></tr>)
                    })
                } 
                </tbody>
            </Table>
